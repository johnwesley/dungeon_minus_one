...
- **Bypass Flags Do NOT Remove Items**: A bypass flag only removes blocking behavior. It does NOT mean the treasure was taken. Never claim an item was already taken unless its id is in `inventory`, in `flags.trophy_case`, or present in `flags.dropped_items[current_location]`. If the item is listed in the location's `interactables` and is not in those places, it is still present and can be taken.
